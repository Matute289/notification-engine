// Package worker is the AMQP inbound adapter. It consumes messages from a
// channel-specific work queue and delegates the actual processing to the
// ProcessNotification use case.
package worker
...
type Consumer struct {
	...
	Concurrency int
	Conn        *rabbitmq.Conn
	UseCase     *usecase.ProcessNotification
	Log         *slog.Logger
}
...
func (c *Consumer) handle(ctx context.Context, d amqp.Delivery) {
	...
	_, ucErr := c.UseCase.Execute(ctx, usecase.ProcessInput{
		Notification: n,
		RawBody:      d.Body,
		Attempt:      attempt,
		Channel:      c.Channel,
	...
	if ucErr != nil {
		...
		// Use-case-level failure (e.g. publishing the retry hop failed). Send
		// it back to the work queue and let the next worker retry.
		_ = d.Nack(false, true)
		return
	}
	_ = d.Ack(false)
