...
- HMAC: clients sign `timestamp \n method \n path \n onBehalfOf \n raw-body` with SHA-256, send `X-App-Key`, `X-App-Timestamp`, `X-App-Signature`, and optionally `X-On-Behalf-Of-User` (the user ID string). `onBehalfOf` is included in the signature even when empty (blank line), so the canonical string is unambiguous.
